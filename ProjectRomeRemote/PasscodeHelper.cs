...
namespace ProjectRomeRemote
{
    public class PasscodeHelper
    {
        ...
        {
            // RnD, TODO
            string password = "B3Z3quGQ";// Membership.GeneratePassword(8,0);
        ...
        {
            // RnD, TODO
            string path = ApplicationData.Current.LocalFolder.Path;//AppDomain.CurrentDomain.BaseDirectory;
        ...
        internal static async Task<bool> Auth(string password,string key)
        {
            ...
            var system = 
                await Windows.System.RemoteSystems.RemoteSystem.FindByHostNameAsync(new Windows.Networking.HostName("127.0.0.1"));
            ...
            if(system!=null)
            {
                if(string.Equals(system.Id,key))
                {
                    ...
                    {
                        using (StreamReader sr = new StreamReader(path + @"\credentials.txt", true))
                        {
