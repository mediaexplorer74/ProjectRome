        ...
        private async void RemoteSystemWatcher_RemoteSystemRemoved(RemoteSystemWatcher sender, RemoteSystemRemovedEventArgs args)
        {
            if(DeviceList != null)
            {
                ...
                (
                    ...
                    {
                        var item = 
                        DeviceList.Where(q => q.UniqueId == args.RemoteSystemId)
                        .FirstOrDefault();
            ...
            if (DeviceList != null)
            {
                await Windows.ApplicationModel.Core.CoreApplication.MainView.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
                {
                    ...
                    (
                        new DeviceDetails() 
                        { 
                            DeviceName = args.RemoteSystem.DisplayName, 
                            UniqueId = args.RemoteSystem.Id, 
                            Type = RemoteSystemHelper.ConvertToDeviceType(args.RemoteSystem.Kind),
                            StatusMessage=RemoteSystemHelper.GetStatusMessage(args.RemoteSystem.Status,args.RemoteSystem.Id), 
                            Credential=DatabaseHelper.GetKey(args.RemoteSystem.Id), 
                            RemoteSys=args.RemoteSystem 
                        }
                    );
                    RaisePropertyChanged("ShowEmptyErrorMessage");
                });
                ...
                await Windows.ApplicationModel.Core.CoreApplication.MainView.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
                {
                    ...
                    (
                        ...
                        { 
                            DeviceName = args.RemoteSystem.DisplayName, 
                            UniqueId = args.RemoteSystem.Id, 
                            Type = RemoteSystemHelper.ConvertToDeviceType(args.RemoteSystem.Kind),StatusMessage = RemoteSystemHelper.GetStatusMessage(args.RemoteSystem.Status, args.RemoteSystem.Id), 
                            Credential = DatabaseHelper.GetKey(args.RemoteSystem.Id), 
                            RemoteSys = args.RemoteSystem 
                        }
                    );
        ...
        public RelayCommand<DeviceDetails> RebootCommand
        {
            get
            {
                return _rebootCommand
                    ...
                    async (x) =>
                    {
                        var res = await RemoteSystemHelper.ExecuteCommand
                        (
                            x.RemoteSys, 
                            new Command() 
                            { 
